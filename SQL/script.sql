...
CREATE TABLE IF NOT EXISTS `licitacion`.`tbl_entidad_adjudicadora` (
  `entidad_adjudicadora` INT NOT NULL AUTO_INCREMENT,
  `expedientes` INT(11) NOT NULL,
  `fecha_actualizacion` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `pais` VARCHAR(3) NULL,
  `nombre_contacto` VARCHAR(300) NULL,
  `telefono` VARCHAR(55) NULL,
  `fax` VARCHAR(55) NULL,
  `correo_electronico` VARCHAR(500) NULL,
  `NIF` VARCHAR(200) NOT NULL,
  PRIMARY KEY (`entidad_adjudicadora`),
  INDEX `fk_tbl_entidad_adjudicadora_tbl_expedientes1_idx` (`expedientes` ASC),
  INDEX `fk_tbl_entidad_adjudicadora_tbl_contracting_party_type_code_idx` (`contracting_party_type_code` ASC),
  INDEX `fk_tbl_entidad_adjudicadora_tbl_country_identification_code_idx` (`pais` ASC),
  CONSTRAINT `fk_tbl_entidad_adjudicadora_tbl_expedientes1`
    FOREIGN KEY (`expedientes`)
    REFERENCES `licitacion`.`tbl_expedientes` (`expedientes`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tbl_entidad_adjudicadora_tbl_contracting_party_type_code1`
    FOREIGN KEY (`contracting_party_type_code`)
...
CREATE TABLE IF NOT EXISTS `licitacion`.`tbl_entidad_adjudicadora_id` (
  `entidad_adjudicadora` INT NOT NULL,
  `id` INT NOT NULL,
